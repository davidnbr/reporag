...
```mermaid
%%{init: {"themeVariables": {"xyChart": {"plotColorPalette": "#d9534f, #5cb85c"}}}}%%
xychart-beta
    title "LLM Response Quality: Baseline (red) vs RAG (green), composite 1-5"
    x-axis ["reporag", "Go project", "Django", "Discovery"]
    y-axis "Composite score" 0 --> 5
    bar "Baseline" [3.98, 2.24, 2.35, 1.80]
    bar "RAG" [4.16, 3.84, 4.25, 4.40]
```
...
```mermaid
xychart-beta
    title "Discovery Mode: Recall@10 by stage"
    x-axis ["grep-discovery", "dense", "bm25", "full"]
    y-axis "Recall@10" 0 --> 1
    bar [0.760, 1.000, 1.000, 1.000]
```
